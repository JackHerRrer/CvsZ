...
int findBestCombo(CHARAC * const hero, int * const humanCount, CHARAC humanList[100], int const zombieCount, CHARAC zombieList[100]){
    ...
    for (double angle = 0; angle  < 360; angle += ANGLE_STEP){
        ...
        for (double distance = 0; distance <= HERO_MVMT; distance += DIST_STEP){
            ...
            int dist = getDistance(tempHero.x, tempHero.y, humanList[0].x, humanList[0].y);
            if (dist > HERO_RANGE){
                // prune this branch
                continue;
            }
...
int main()
{
    ...
    while (1) {
        ...
        int score = findBestCombo(&hero, humanCount, humanList, zombieCount, zombieList);
